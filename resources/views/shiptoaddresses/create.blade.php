...
@section('container')
    ...
    <div class="col-lg-4">
        <form method="post" action="/shiptoaddresses" class="mb-5">
            @csrf
            <div class="mb-3">
                @foreach ($shiptoaddresses as $shiptoaddress)
                    @if ($loop->last)
                        <label for="ship_id" class="col-form-label">Ship ID:</label>
                        <input type="name" class="form-control rounded-top @error('ship_id') is-invalid @enderror"
                            name="ship_id" id="ship_id" placeholder="ship_id" readonly
                            value="{{ $shiptoaddress->ship_id + 1 }}">
                    @endif
                @endforeach
            </div>
            <div class="mb-3">
            ...
            <h6 class="mt-5">Input Alamat Kirim</h6>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="name" class="form-control rounded-top @error('alamat_kirim3') is-invalid @enderror"
                    name="alamat_kirim3" id="alamat_kirim3" placeholder="Input Kota" value="{{ old('alamat_kirim3') }}">
                @error('alamat_kirim3')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
