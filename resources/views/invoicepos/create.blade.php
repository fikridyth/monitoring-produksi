...
@section('container')
    ...
    <div class="col-lg-4">
        <form method="post" action="/invoicepos" class="mb-5">
            @csrf
            <div class="mb-3">
                <label for="purchaseorder" class="form-label">Pilih Kode PO:</label>
                    ...
                    @foreach ($purchaseorders as $purchaseorder)
                        @if (old('purchaseorder_id') == $purchaseorder->id)
                            <option value="{{ $purchaseorder->id }}" selected>
                                {{ $purchaseorder->no_po }}{{ $purchaseorder->no_item }}
                            </option>
                        @else
            ...
            <a href="/invoicepos" class="btn btn-secondary col-lg-3">Back</a>
